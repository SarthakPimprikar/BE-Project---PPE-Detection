...
class Monitor:
    def __init__(self, args: argparse.Namespace):
        ...
        # Person (only if requested, otherwise rely on unified model)
        if hasattr(args, 'person_weights') and args.person_weights:
            self.person_det = PersonDetector(weights=args.person_weights, conf=args.person_conf)
        else:
            self.person_det = None
            logger.info("Person-specific model disabled (Unified mode).")
        # PPE backend
    ...
    def _split_classes(self, boxes: List[Box]) -> Tuple[List[Box], List[Box], List[Box]]:
        helmets, vests, persons = [], [], []
        for b in boxes:
            lbl = b.label.lower()
            if lbl in ("helmet", "hardhat", "hard-hat"):
                if b.conf >= self.assoc_cfg.conf_helmet:
                    helmets.append(b)
            elif lbl in ("vest", "jacket", "reflective_jacket", "reflective_vest", "safety vest", "safety-vest"):
                if b.conf >= self.assoc_cfg.conf_vest:
                    vests.append(b)
            elif lbl == "person":
                persons.append(b)
        return helmets, vests, persons
    ...
    def process_frame(self, frame: np.ndarray) -> Tuple[np.ndarray, Dict[str, Any]]:
        # 1. Unified Inference Pass (Optimized)
        all_detections = self.ppe_backend.infer(frame)
        all_detections = self._filter_boxes(all_detections)
        
        # 2. Extract PPE and Persons from the single pass
        helmets, vests, persons_v1 = self._split_classes(all_detections)
        
        # 3. Fallback to separate Person Detector ONLY if no persons were found in PPE pass
        if not persons_v1 and self.person_det:
             persons_raw = self.person_det.detect(frame)
        else:
             persons_raw = persons_v1
    ...
    def run_stream(self):
        ...
        while True:
            ...
            if not ret:
                break
            out, _ = self.process_frame(frame)
